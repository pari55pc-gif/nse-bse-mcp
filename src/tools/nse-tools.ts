...
export const nseTools: Tool[] = [
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description:
            'Company name or symbol search query.',
        },
      },
      required: ['query'],
    },
...

  {
    ...
    description:
      'Get top gainers from the NIFTY 50 market data universe.',
    inputSchema: {
      type: 'object',
      properties: {
        count: {
...

  {
    name: 'nse_get_losers',
    description:
      'Get top losers from the NIFTY 50 market data universe.',
    inputSchema: {
      type: 'object',
      properties: {
        count: {
...

  {
    ...
    description:
      'Fetch historical F&O data for backtesting. Supports futures/options filtering by instrument type, expiry, option type and strike price.',
    inputSchema: {
      type: 'object',
      properties: addFilterProperties({
        symbol: {
          type: 'string',
          description:
            'F&O symbol, e.g. RELIANCE, SBIN, TCS.',
        },
        from_date: {
          type: 'string',
          description:
    ...
    inputSchema: {
      ...
      properties: addFilterProperties({
        ...
        stocks_only: {
          type: 'boolean',
          description:
            'Return only stock F&O contracts FUTSTK and OPTSTK. Default true.',
          default: true,
        },
        ...
        instrument_type: {
          type: 'string',
          description:
            'Optional stock F&O instrument filter.',
          enum: [
            'FUTSTK',
            'OPTSTK',
          ],
...

  {
    ...
    description:
      'Fetch historical NSE India VIX data.',
    inputSchema: {
      type: 'object',
      properties: addFilterProperties({
        from_date: {
...

  {
    ...
    description:
      'Get complete option-chain data including CE/PE price, OI, change in OI, IV and volume.',
    inputSchema: {
      type: 'object',
      properties: addFilterProperties({
        symbol: {
...

  {
    ...
    description:
      'Get compact option-chain data around ATM strikes.',
    inputSchema: {
      type: 'object',
      properties: addFilterProperties({
        symbol: {
...

  {
    ...
    description:
      'Download NSE daily equity bhavcopy for a specific date.',
    inputSchema: {
      type: 'object',
      properties: addFilterProperties({
        date: {
...

  {
    ...
    description:
      'Download NSE F&O bhavcopy for a specific trading date.',
    inputSchema: {
      type: 'object',
      properties: addFilterProperties({
        date: {
